...
# Define circular tumor properties
tumor_radius = int(image_size / 6)
tumor_center = (int(image_size / 2), int(image_size / 2), int(image_size / 2))

# Define tiny circle properties within the tumor. Use uptake range to determine heterogeneity
num_circles = 3000
...
# Generate the tiny circles within the tumor
for _ in range(num_circles):
    ...
    # Generate random circle position within the tumor
    while True:
...
# Generate a unique filename for the DICOM file
filename_pet = "PSF_0_PAPERFIG.dcm"
...
#plt.subplot(1, 3, 1)
#plt.imshow(coronal_slice, cmap="hot")
#plt.title("Coronal Slice")
#plt.axis("off")

#plt.subplot(1, 3, 2)
#plt.imshow(sagittal_slice, cmap="hot")
...
plt.tight_layout()
plt.savefig('PAPERFIG6.0.png', dpi=300)
plt.show()